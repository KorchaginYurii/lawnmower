...
Stage2 - train imitation hl, который обучит RL policy копировать teacher policy

===== RL FINE TUNING =====
Потому что:
	RL стартует НЕ с random policy,
	а с уже хорошей teacher policy
Это огромная разница.

===== HYBRID POLICY =============
rule policy
	+
RL confidence override

rule policy = safe deterministic backbone
RL = adaptive optimizer
...
self.current_cell = None найден тут 
lawn_sweep_agent.py
__init__()
...
act_go_to_sector()
    if target is None:
        ...
        return WAIT_ACTION	
act_sweep_sector()
	if self.mission.cell_uncut(env, self.decomposition, self.current_cell,) <= 0:
		self.mission.finish_cell(self.current_cell)
        self.current_cell = None
...
boustrophedon_mission_planner.py
__init__()
	self.current_cell = None
reset()
	self.current_cell = None
